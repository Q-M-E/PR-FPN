...
class PRFPN_4(nn.Module):
    def __init__(self, inter_dim=512, level=0, channel=None):
        ...
        if self.level == 0:
            ...
            lateral_norm = get_norm(norm='', out_channels=channel[3])
...
class Body(nn.Module):
    def __init__(self, channels=None):
        ...
        self.prfpn_4_level0 = PRFPN_4(inter_dim=channels[0], level=0, channel=channels)
        self.prfpn_4_level1 = PRFPN_4(inter_dim=channels[1], level=1, channel=channels)
        self.prfpn_4_level2 = PRFPN_4(inter_dim=channels[2], level=2, channel=channels)
        self.prfpn_4_level3 = PRFPN_4(inter_dim=channels[3], level=3, channel=channels)
...
class PRFPN(nn.Module):
    def __init__(self,
        ...
        in_channels_reduced = [i // 4 for i in in_channels]
